...
const WeatherService = () => {
  ...
  useEffect(() => {
    ...
    const getForecastData = async () => {
      try {
        let response = await fetch(
          `https://api.openweathermap.org/data/2.5/forecast?q=${city}&APPID=2469ed517ab38109f0124ab104977c4a&units=metric`
        );
        if (response.ok) {
          const data = await response.json();
          console.log("data ", data);
  ...
  //       `https://api.openweathermap.org/data/2.5/weather?q=${searchedCity}&appid=2469ed517ab38109f0124ab104977c4a&units=metric`
  //     );
